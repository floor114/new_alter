...
describe Request::Confirmed::Index do
  let(:user) { create :user }
  let(:params) { {} }
  let!(:request1) { create :request, :confirmed, user: user }
  let!(:request2) { create :request, :confirmed, user: user }
  let!(:request3) { create :request, :confirmed }
  ...
  context 'when user id is' do
    context 'present' do
      before { request1.update(created_at: Time.zone.now + 5.minutes) }
